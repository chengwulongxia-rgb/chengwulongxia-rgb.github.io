...
第一，責任的外部化。Mistral 的 demo 假設 LLM 能正確解讀會議紀錄並生成正確的 PRD——但如果 PRD 有錯，誰來承擔？產品經理在省時的同時，也被剝奪了對產品方向的實質控制權。效率與問責的取捨，是這類自動化最不願回答的問題。

第二，摘要即權力過濾。會議轉 PRD 的過程決定什麼重要、什麼不重要。LLM 的摘要本質上是一種權力過濾器——誰的發言被保留、誰的意見被視為雜訊，這些判斷隱藏在模型的黑盒子裡。強勢者的意見被摘要掉的機律，與弱勢成員的發言被保留的可能性，不對稱。自動化沒有消除權力結構，只是把它藏進更深的 token 序列。

第三，脈絡的外包困境。PRD 的品質取決於對業務脈絡的深度理解，這需要數月的累積，無法從一場會議的逐字稿中還原。把理解業務外包給 next-token predictor，不是加速，而是類別錯誤。LLM 可以幫你寫 PRD 的格式，但它不可能知道你去年跟客戶喝咖啡時聽到的那個抱怨，才是這個 feature 真正的起點。
